...
public class U {
    ...
    public static void printfileInfo(MultipartFile file) {
        ...
        System.out.println("""
                ...
                """.formatted(
                        originalFileName,
                        StringUtils.cleanPath(originalFileName), // 파일명에서 / 와 \ 가 섞여있을 때 "/" 로 전부 바꿔줌.
        ...
        // 이미지 파일 여부
        try {
            BufferedImage bufferedImage = ImageIO.read(file.getInputStream());  // 이미지면 bufferedImage 객체를 리턴, 이미지가 아니면 null 리턴
